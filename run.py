...
DATA_PATH = "mfcc_16000_40_1103_160_data.json"
...
"""
处理为dataset：train, validation and test
"""
...
def train(model, epochs, batch_size, patience, x_train, y_train, x_validation,
    ...
    :param patience (int): Num epochs to wait before early stop, if there isn't
                           an improvement on accuracy # 如果在验证集上的准确度没有提升，等待停止训练的轮数
    ...
    class CustomCallback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs=None):
            with open(RESULTS_PATH, "a") as f:
                ...
                f.write(f"Training loss: {logs['loss']}, Training accuracy: {logs['accuracy']}\n")
                f.write(f"Validation loss: {logs['val_loss']}, Validation accuracy: {logs['val_accuracy']}\n\n")
...
def calculate_attack_accuracy(model, x_test, y_test, f_test, trigger,
    ...
    with open(RESULTS_PATH, "a") as file:
        file.write(f"Attack accuracy: {attack_acc}")
    return attack_acc
...
def run_experiments():
    ...
    for arch in ["dense"]:
        for arch_name in ["adv_detection"]:
            # ["trojaning_attacks", "adv_detection", "lstm_att"]
            for epochs in [2]:
                # for _ in range(1):
